...
// MADE BY  [  BIRINDER SINGH WARRAICH ] , BCA STUDENT , PUNJAB TECHNICAL UNIVERSITY with help from teachers and friend.
...
// To access employee class password is 3698, this is largely to implement the concept that only university staff or admnins can access employee menu.
...
class student_system{
	...
	public:
		char student_name[100];        // Public data memebers.
...
// arguments passed : This entire function ask user to input data in student_system stu[100} object array. This is connected with case 1 of nested switch case 1:{   
	   	        ...
	   	        std::cout <<"NOTE: DO NOT ENTER DATA WITH SPACES USE _ ONLY [PRESS ENTER KEY]\n";
	   	        std::cin.ignore();
	   	        std::cin.get();
            ...
            for(int i = 0;i<n;i++){
	   	    ...
	   	    std::cin >> stu[i].student_name;
	   	    std::cout <<std::endl;
	   	    
	   	    std::cout <<"Enter Father Name of :[" <<stu[i].student_name <<"]" <<"\n";
	   	    std::cin >> stu[i].father_name;
	   	    std::cout <<std::endl;
...
// The below function display information of student only if the name of the student and password that was created is correct. We pass arguments in nested switch case 1{ case 2: stu[i]. for comparison	
	   ...
	   int display_student_data(const char name1[100],const char password1[100],int &n , student_system stu[100]){
	   	for(int i=0;i<n;i++){
		   
	   	if(strcmp(name1,stu[i].getname())==0){
	   		if(strcmp(password1,stu[i].getpassword())==0){
	   ...
	   int grades_registeration(const char name3[100],int &n,student_system stu[100],const char password3[100]){
	   	for(int i=0;i<n;i++){
	   		if(strcmp(name3,stu[i].getname())==0){
	   			if(strcmp(password3,stu[i].getpassword())==0){
	   ...
	   int generate_report_card(const char name4[100],int &n,  student_system stu[100],const char password1[100]){
	   	for(int i=0;i<n;i++){
	   		if(strcmp(name4,stu[i].getname())==0){
	   			if(strcmp(password1,stu[i].getpassword())==0){
	...
	int fee_payments(const char name5[100],int &n, int &n1, student_system stu[100],const char password1[100]){
		for(int i=0;i<n;i++){
			
			if(strcmp(name5,stu[i].getname())==0){
	...
	int display_payment_history(const char name6[100],int &n ,int &n1, student_system stu[100],const char password[100]){
		for(int i = 0;i<n;i++){
			if(strcmp(name6,stu[i].getname())==0){
				if(strcmp(password,stu[i].getpassword())==0)
				std::cout <<"------------------------------------------------\n";
	...
	int delete_student_data(const char name[100],int &n, student_system stu[100],const char password1[100]){
		...
		return 0;
		    ...
		    const char* getname(){   // function that used pointer to return student name so it can be used in object arrays and for better comparison overall
	   	return student_name;
	   }
	   
	   const char* getpassword(){  // functions that uses pointer to return password so it can be used in comaprison to bring out the result 
	   	return password;
	   }
...
class employee{
	  ...
	  public:
	  	char employee_name[100];      // decalring public data types as there values can be intialized 
	  	char job_name[100];
	  	char password12[100];
		...
		int input_employee_data(int &n, employee emp[100]){                                           // function that inputs employee information and stores in object arrays here emp[100], v
			std::cout <<"NOTE: DO NOT ENTER DATA WITH SPACES USE _ ONLY [PRESS ENTER KEY]\n";
			...
			std::cin >> emp[i].employee_name;
			std::cout <<std::endl;
			
			std::cout <<"Job Name of Employee :[" <<emp[i].employee_name <<"]" <<"\n";
			std::cin >> emp[i].job_name;
		    std::cout <<std::endl;
		...
		int display_employee_info(const char emp_name[100],int &n , employee emp[100],const char password[100]){    // function to just displays employee information 
			for(int i = 0;i<n;i++){
				if(strcmp(emp_name,emp[i].get_emp_name())==0){
					if(strcmp(password,emp[i].get_password())==0){
		...
		int change_password(const char name[100],int &n , employee emp[100],const char password[100]){        
			for(int i=0;i<n;i++){
				if(strcmp(name,emp[i].get_emp_name())==0){
					if(strcmp(password,emp[i].get_password())==0){
					std::cout <<"--------------------------\n";
	...
	int change_employee_id(const char name[100],int &n , employee emp[100],const char password[100]){
			for(int i=0;i<n;i++){
				if(strcmp(name,emp[i].get_emp_name())==0){
					if(strcmp(password,emp[i].get_password())==0){
					std::cout <<"----------------------------\n";
	...
	// sames as above 
	int change_employee_email(const char name[100],int &n , employee emp[100],const char password[100]){
			for(int i=0;i<n;i++){
				if(strcmp(name,emp[i].get_emp_name())==0){
					if(strcmp(password,emp[i].get_password())==0){
					std::cout <<"-------------------------\n";
	...
	int promotion_salary_hike(const char name[100], int &n , employee emp[100],const char password[100]){
		for(int i = 0 ;i<n;i++){
			if(strcmp(name,emp[i].get_emp_name())==0){
				if(strcmp(password,emp[i].get_password())==0){
	...
	int delete_emp_info(const char name[100],int &n , employee emp[100], const char password[100]){
		for(int i=0;i<n;i++){
			if(strcmp(name,emp[i].get_emp_name())==0){
				if(strcmp(password,emp[i].get_password())==0){
					std::cout <<"Are you sure you want to delete [PRESS ENTER KEY TO CONFIRM]\n";
...
int employee_password = 3698;   // password to access employee menu. this variable has been declared globally 
	...
	do{
		...
		switch (choice){     //outere loop 
			...
			case 1:{
				...
				char name[100]; // variable to compare password in stu() arguments here and then in the class
				
				int result; // used to store the value of task 
				
				char password[100];  // variable to compare password in stu() arguments here and then in the class
				...
				do{
					...
					switch (inner_choice){            // inner switch case of nested switch case
						...
						case 3:
						 ...
						 result = stu[i].grades_registeration(name,n,stu,password);  // arguments passed here accordingly
						 if(result == 1){
						 	break;
						 }break;
					   ...
					   case 4:
					   	...
					   	std::cout <<"Enter Password :\n";
						std::cin >> password;
					   	result = stu[i].generate_report_card(name,n,stu,password); // arguments passed here accordingly
					   	if(result == 1){
					   		break;
						   }
						   break;
			...
			case 2:{                  // case 2: { for employee menu starts here
				...
				int password; // this variable is being used to compare globally decalred employee_password with password to start employee meny
				
				char password1[100];   // comparison for password
				...
				if(employee_password==password){
					...
					switch(second_choice){      // case 2:{ inner switch or nested switch begins here
						...
						case 1:
							...
							emp[i].input_employee_data(n,emp);       // calls the function
							break;
						...
						std::cout <<std::endl;
						 ...
						 result = emp[i].display_employee_info(name,n,emp,password1);
						 if(result == 1){
						 	break;
						 }break;
							     ...
							     switch (case_3_choice){   // choice for this particular nested case
							     	...
							     	case 1:
							            ...
							            std::cout <<std::endl;
							     		...
							     		result = emp[i].change_password(name,n,emp,password1);      // calls the change password functionm
							     		if(result == 1){
							     			break;
										 }break;
								     ...
								     case 2:
									  ...
									  std::cout <<"Enter Password:\n";
						           	  std::cin >> password1;
							          std::cout <<std::endl;
									  result = emp[i].change_employee_id(name,n,emp,password1);
									  if(result == 1){
									  	break;
									  }break;
									...
									std::cout <<std::endl;
									 ...
									 result = emp[i].change_employee_email(name,n,emp,password1);
									 if(result == 1){
									 	break;
									 }break;
						...
						case 4:
							...
							result = emp[i].promotion_salary_hike(name,n,emp,password1);
							if(result == 1){
								break;
							}break;
						...
						case 5:
							...
							result = emp[i].delete_emp_info(name,n,emp,password1);
							if(result == 1){
									std::cout <<"ACCOUNT HAS BEEN DELETED :--\n";
					  	            std::cout <<"----------------------------\n";
								break;
